...
from nomaj.maybe import Maybe, err_
...
class NmAuth(Nomaj):
    ...
    async def act_on(self, request: Req) -> Maybe[Resp]:
        user: Maybe[Identity] = await self._pass.enter(request)
        if user.err():
            return err_(user)
        if user != ANONYMOUS:
    ...
    async def act_identified_on(self, request: Req, identity: Identity) -> Maybe[Resp]:
